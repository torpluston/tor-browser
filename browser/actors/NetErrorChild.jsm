...
class NetErrorChild extends ActorChild {
  ...
  _setTechDetails(input, doc) {
    ...
    let msg1 = gPipNSSBundle.formatStringFromName(
      "certErrorIntro",
      [hostString],
      1
    );
      ...
      switch (input.data.code) {
        case MOZILLA_PKIX_ERROR_MITM_DETECTED:
          if (mitmErrorPageEnabled) {
            ...
            msg1 += "\n\n";
            msg1 += gPipNSSBundle.formatStringFromName(
              "certErrorMitM2",
              [brandName],
              1
            );
        ...
        case MOZILLA_PKIX_ERROR_ADDITIONAL_POLICY_CONSTRAINT_FAILED:
          msg1 +=
            gPipNSSBundle.GetStringFromName("certErrorTrust_Symantec1") + "\n";
          break;
        default:
          msg1 +=
    ...
    if (input.data.isNotValidAtThisTime) {
      let msg;
      if (
        input.data.validity.notBefore &&
        Date.now() < input.data.validity.notAfter
      ) {
        let notBeforeLocalTime = formatter.format(
          new Date(input.data.validity.notBefore)
        );
        msg = gPipNSSBundle.formatStringFromName(
          "certErrorNotYetValidNow3",
          [hostString, notBeforeLocalTime],
          2
        );
      } else {
        let notAfterLocalTime = formatter.format(
          new Date(input.data.validity.notAfter)
        );
        msg = gPipNSSBundle.formatStringFromName(
          "certErrorExpiredNow3",
          [hostString, notAfterLocalTime],
          2
        );
      }
      msg += "\n";
  ...
  onCertErrorDetails(msg, docShell) {
    ...
    function updateContainerPosition() {
      ...
      if (doc.ownerGlobal.parent == doc.ownerGlobal) {
        ...
        let offset =
          doc.documentElement.clientHeight / 2 - textContainer.clientHeight / 2;
        if (offset > 0) {
    ...
    let errWhatToDo = doc.getElementById(
      "es_nssBadCert_" + msg.data.codeString
    );
    ...
    switch (msg.data.code) {
      ...
      case MOZILLA_PKIX_ERROR_ADDITIONAL_POLICY_CONSTRAINT_FAILED:
        ...
        adminDescription.textContent = gPipNSSBundle.GetStringFromName(
      ...
      case MOZILLA_PKIX_ERROR_MITM_DETECTED:
        if (mitmErrorPageEnabled) {
          let autoEnabledEnterpriseRoots = Services.prefs.getBoolPref(
      ...
      case MOZILLA_PKIX_ERROR_NOT_YET_VALID_ISSUER_CERTIFICATE:
        ...
        let difference = Services.prefs.getIntPref(
          PREF_SERVICES_SETTINGS_CLOCK_SKEW_SECONDS,
          0
        );
        let lastFetched =
          Services.prefs.getIntPref(PREF_SERVICES_SETTINGS_LAST_FETCHED, 0) *
          1000;
        ...
        if (clockSkew) {
          ...
          // eslint-disable-next-line no-unsanitized/property
          doc.querySelector(".title-text").textContent =
          ...
          let targetElems = doc.querySelectorAll(
          ...
          let errDesc = doc.getElementById(
  ...
  receiveMessage(msg) {
    if (msg.name == "CertErrorDetails") {
      let frameDocShell = WebNavigationFrames.findDocShell(
        msg.data.frameId,
        this.docShell
      );
  ...
  onPageLoad(originalTarget, win) {
    ...
    if (this.isAboutCertError(win.document)) {
      ...
      hideAddExceptionButton = Services.prefs.getBoolPref(
